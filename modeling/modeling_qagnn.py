...
class QAGNN(nn.Module):
    def __init__(self, args,pre_dim, k, n_ntype, n_etype, sent_dim,
        ...
        self.svec2nvec = nn.Linear(sent_dim, concept_dim) # 现在使用的是 DL模型和
        ...
        self.pooler = MultiheadAttPoolLayer(n_attention_head, sent_dim, concept_dim) # 混合多层注意力机制
        ...
        self.fc = MLP(concept_dim + sent_dim*2 + concept_dim, fc_dim, self.pre_dim, n_fc_layer, p_fc, layer_norm=True)
    ...
    def forward(self, sent_vecs, retain_vec, concept_ids, node_type_ids, node_scores, adj_lengths, adj, emb_data=None, cache_output=False):
        """
        ...
        concat = self.dropout_fc(torch.cat((graph_vecs, sent_vecs, Z_vecs, retain_vec), 1))
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, t_diag, t_mask, *inputs, layer_id=-1, cache_output=False, detail=False):
        """
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs # concept_ids来源
        ...
        vecs_retain = self.encoder_Retain( t_diag, t_mask ) # bsz*768
        vec_pretrain, atten_pretrain = self.encoder_PreTrain(*lm_inputs) # vec_pretrain: bsz*768
        # sent_vec = torch.cat( (vecs_retain, vec_pretrain), 1) # sent_vec: bsz*768+768
        sent_vec = vec_pretrain
        # sent_vecs, all_hidden_states = self.encoder(*lm_inputs, layer_id=layer_id)
        logits, attn = self.decoder(sent_vec.to(node_type_ids.device),
                                    vecs_retain.to(node_type_ids.device),
                                    concept_ids,
...
class LM_QAGNN_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
        ...
        # 加载训练、验证集为tensor张量
        # 先由文本examples到features格式，再到id数据，再加载为张量
        # qid：数据、label的统一标识
        # labels：将文本或其它形式label处理成id
        # data：all_input_ids, all_input_mask, all_segment_ids, all_output_mask
        self.train_qids, self.train_HF_labels, self.train_Diag_labels, self.train_t_diagnosis_codes, \
            self.train_t_mask, *self.train_encoder_data = load_input_tensors(train_statement_path, model_type, model_name, max_seq_length)
        self.dev_qids, self.dev_HF_labels, self.dev_Diag_labels, self.dev_t_diagnosis_codes, \
            self.dev_t_mask, *self.dev_encoder_data = load_input_tensors(dev_statement_path, model_type, model_name, max_seq_length)
        # 选项数目
        num_choice = self.train_encoder_data[0].size(1)
        self.num_choice = num_choice
        print ('num_choice:', num_choice)
        # train_decoder数据来源于train_adj_path，但在原始数据处理中，检查过没有超限的
        ...
        *self.dev_decoder_data, self.dev_adj_data = load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, num_choice, args)
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_HF_labels, self.test_Diag_labels, self.test_t_diagnosis_codes, \
            self.test_t_mask, *self.test_encoder_data = load_input_tensors(test_statement_path, model_type, model_name, max_seq_length)
