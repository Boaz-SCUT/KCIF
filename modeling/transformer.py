...
class EncoderNew(nn.Module):
    ...
    def forward(self, diagnosis_codes, mask, mask_code, seq_time_step, input_len): # diagnosis_codes: torch.Size([12, 6, 32])
        # diagnosis_codes = diagnosis_codes.cuda()
        # mask_code = mask_code.cuda()
        seq_time_step = torch.Tensor(seq_time_step).cuda().unsqueeze(2) / 180 #
        seq_time_step = self.selection_layer(seq_time_step)
        time_feature = 1 - self.tanh(torch.pow(seq_time_step, 2))
        time_feature = self.time_layer(time_feature) # torch.Size([16, 5, 768])
        output = (self.pre_embedding(diagnosis_codes) * mask_code).sum(dim=2) + self.bias_embedding # torch.Size([12, 6, 768])
        output += time_feature # diag + time
        output_pos, ind_pos = self.pos_embedding(input_len.unsqueeze(1))
        output += output_pos
        self_attention_mask = padding_mask(ind_pos, ind_pos)
...
class TransformerTime(nn.Module):
    ...
    def forward(self, seq_dignosis_codes, seq_time_step, options, maxlen):
        ...
        seq_time_step = np.array(list(units.pad_time_hita(seq_time_step, options)))
        lengths = torch.from_numpy(np.array([len(seq) for seq in seq_dignosis_codes])).cuda()
        diagnosis_codes, mask, mask_final, mask_code = units.pad_matrix_new(seq_dignosis_codes, options)
        if options['use_gpu']:
