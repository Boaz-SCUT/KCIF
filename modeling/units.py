...
def adjust_input_hita(batch_diagnosis_codes, batch_time_step, max_len, n_diagnosis_codes,
                      batch_time_step2, main_codes, sub_codes1, sub_codes2):
    for ind in range(len(batch_diagnosis_codes)):
        if len(batch_diagnosis_codes[ind]) > max_len:
            batch_diagnosis_codes[ind] = batch_diagnosis_codes[ind][-(max_len):]
            main_codes[ind] = main_codes[ind][-(max_len):]
            sub_codes1[ind] = sub_codes1[ind][-(max_len):]
            sub_codes2[ind] = sub_codes2[ind][-(max_len):]
            batch_time_step[ind] = batch_time_step[ind][-(max_len):]
            batch_time_step2[ind] = batch_time_step2[ind][-(max_len):]
        batch_time_step[ind].append(0)
        batch_time_step2[ind].append(0)
        batch_diagnosis_codes[ind].append([n_diagnosis_codes])
        main_codes[ind].append([939])
        sub_codes1[ind].append([11])
        sub_codes2[ind].append([11])
    return batch_diagnosis_codes, batch_time_step, batch_time_step2, main_codes, sub_codes1, sub_codes2

def adjust_input_hita2(batch_diagnosis_codes, batch_time_step, max_len, n_diagnosis_codes,
                      batch_time_step2):
    for ind in range(len(batch_diagnosis_codes)):
        if len(batch_diagnosis_codes[ind]) > max_len:
            batch_diagnosis_codes[ind] = batch_diagnosis_codes[ind][-(max_len):]
...
def pad_matrix_new(seq_diagnosis_codes, n_diagnosis, maxlen, main_codes, sub_codes1, sub_codes2):
    lengths = np.array([len(seq) for seq in seq_diagnosis_codes])
    n_samples = len(seq_diagnosis_codes)
    n_diagnosis_codes = n_diagnosis
    lengths_code = []
    ...
    batch_diagnosis_codes = np.zeros((n_samples, maxlen, maxcode), dtype=np.int64) + n_diagnosis_codes
    batch_mask = np.zeros((n_samples, maxlen), dtype=np.float32)  # (6000, 21)
    batch_mask_code = np.zeros((n_samples, maxlen, maxcode), dtype=np.float32)  # (6000, 21, 39)
    batch_mask_final = np.zeros((n_samples, maxlen), dtype=np.float32)  # (6000, 21)
    batch_main_codes = np.zeros((n_samples, maxlen, maxcode), dtype=np.int64) + 939
    batch_sub_codes1 = np.zeros((n_samples, maxlen, maxcode), dtype=np.int64) + 11
    batch_sub_codes2 = np.zeros((n_samples, maxlen, maxcode), dtype=np.int64) + 11

    for bid, seq in enumerate(seq_diagnosis_codes):
        for pid, subseq in enumerate(seq):
            for tid, code in enumerate(subseq):
                batch_diagnosis_codes[bid, pid, tid] = code
                batch_main_codes[bid, pid, tid] = main_codes[bid][pid][tid]
                batch_sub_codes1[bid, pid, tid] = sub_codes1[bid][pid][tid]
                batch_sub_codes2[bid, pid, tid] = sub_codes2[bid][pid][tid]
                batch_mask_code[bid, pid, tid] = 1
    # for bid, seq in enumerate(main_codes):
    #     for pid, subseq in enumerate(seq):
    #         for tid, code in enumerate(subseq):
    #             batch_main_codes[bid, pid, tid] = code
    # for bid, seq in enumerate(sub_codes1):
    #     for pid, subseq in enumerate(seq):
    #         for tid, code in enumerate(subseq):
    #             batch_sub_codes1[bid, pid, tid] = code
    # for bid, seq in enumerate(sub_codes2):
    #     for pid, subseq in enumerate(seq):
    #         for tid, code in enumerate(subseq):
    #             batch_sub_codes2[bid, pid, tid] = code
    ...
    return batch_diagnosis_codes, batch_mask, batch_mask_final, batch_mask_code, batch_main_codes, batch_sub_codes1, batch_sub_codes2

def pad_matrix_new2(seq_diagnosis_codes, n_diagnosis, maxlen):
    lengths = np.array([len(seq) for seq in seq_diagnosis_codes])
    n_samples = len(seq_diagnosis_codes)
    n_diagnosis_codes = n_diagnosis
    lengths_code = []
...
def mask_diagnosis_codes(batch_diagnosis_codes, mask_prob, pad_value, mask_value):
    ...
    # 创建一个mask tensor，其中的值为True的位置将被mask
    ...
    # 生成masked序列，如果mask为True则替换为mask_value，否则保持原值
